<template>
  <el-collapse v-model="activeNames" @change="handleChange" class="collapse-list-container">
    <el-collapse-item  v-for="item in dataCollapse"  :key="item.dateTime" :name="item.dateTime">
      <template slot="title">
        {{item.dateTime}} <span class="fr collapse-item-title">给<i>{{item.roomCount}}</i>个直播间送出<i>{{item.barNum}}</i>条弹幕</span>
      </template>
      <barrage-collapse-content :ref="item.dateTime" :data-content = 'contentList[item.dateTime]'></barrage-collapse-content>
    </el-collapse-item>
  </el-collapse>
</template>
...
<script>
  import barrageCollapseContent from './barrageCollapseContent.vue'
  export default {
    components:{
      barrageCollapseContent
    },
    props: {
      dataCollapse: {
        type: Array
