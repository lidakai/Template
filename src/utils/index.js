...
export function Arrangement(dataList) {
  function sort(arr) {
    for(let i=0;i<arr.length-1;i++){
      for(let j=0;j<arr.length-1-i;j++){
        if(arr[j].value<=arr[j+1].value){
  ...
  let sortList = sort(JSON.parse(JSON.stringify(dataList)));
  let data = JSON.parse(JSON.stringify(dataList));
  function getIndex(array,key){
    let num = 0;
    array.forEach((val,index)=>{
      if(sortList[key].value === val.value){
        num = index
      }
    })
    return num;
  }
  let onCodeList = getIndex(sortList,3),toCodeList = getIndex(sortList,onCodeList+4);
  /**
   * 获取到两个关键节点，分别是数量最大的四个小时(包括与第四个小时相同的若干item)，以及往后推四个小时(同上);
   *
   * **/
  sortList.forEach((v,k)=>{
    data.forEach((value,index)=>{
      let color = '';
      if(0<=k&&k<=onCodeList){
        color = '#67a3ff'
      }else if(onCodeList<k&&k<=toCodeList){
        color = '#c2daff'
      }else {
        color = '#e8f1ff'
      }
      if(value.id === v.id){
        value.itemStyle.normal.color = color
      }
    })
  })
  return data;
...
export function bindThird(type,uid) {
  if(isExperience()){
    this.$message("当前为演示账号，请注册登录后使用绑定功能");
    return;
  }
  let str = '';
  if(uid){
...
export function timeToHour(seconds){
  seconds = typeof seconds ? seconds : Number(seconds);
