        ...
        <div class="vicp-wrap">
            ...
            <div class="vicp-step2" v-if="step == 2">
                ...
                <div class="vicp-operate">
                    ...
                    <a class="vicp-operate-btn" @click="uploadForm" @mousedown="ripple">{{ lang.btn.save }}</a>
                </div>
            </div>
...
<script>
    /* eslint-disable */
    import {effectRipple, data2blob,toBlob,b64toBlob} from './utils';
...
//                    }
            ...
            async uploadForm(){
              const response = await api_zbb.service.ZhibobaoPersonalAccount.Account.ossAuth();
              if(response === false) return ;
              console.log(response);
              let obj = response.data.data;
              const blob = data2blob(this.createImgUrl, this.mime);
              // blob转arrayBuffer
              ...
              reader.onload =  (event) =>{
                ...
                const client = new oss.Wrapper({
                  ...
                  bucket: 'zhibobao-image'
                });
                // 文件名
                const date = new Date();
                const time = '' + date.getFullYear() + (date.getMonth() + 1) + (date.getDate());
                const storeAs = 'Uploads-file/' + time + '/' + date.getTime() + '.' + blob.type.split('/')[1];
                ...
                client.put(storeAs, buffer).then( (result)=> {
                  ...
                  this.$emit("on-image-upload",result.url)
                }).catch(function (err) {
                  console.log(err);
                });
              };
...
//                uploadProgress (event);
//                const buffer = new oss.Buffer(event.target.result);
//                uploadAvatar({image:buffer}).then(response=>{
//                  console.log(response);
//                  if(response.data.code==0){
//                    that.loading = 2;
//                    that.$emit('modifyAvatarSuccess', response.data.data.url);
//                  }
//                }).catch(err=>{
//                  if (that.value) {
//                    that.loading = 3;
